...
(defcustom py-syntactic-triple-quoted-strings t
  "If true, then use `font-lock-mode's syntactic keyword feature to
ensure that triple quoted strings are displayed correctly.  In
particular, modify the syntax designation of any quote mark that
should not be a considered string delimiters because it occurs inside
a triple-quoted string.  See `py-font-lock-syntactic-keywords'."
  :type 'boolean  :group 'python)
...
;; python-mode to handle triple-quoted strings correctly.
 
(defvar py-font-lock-syntactic-keywords
  '((py-quote-in-triple-quoted-string-matcher 0 (1 . nil)))
  "Syntactic keyword table for Python, which finds quote marks that
...
(defun py-quote-in-triple-quoted-string-matcher (limit)
  "A `font-lock-mode' MATCHER that searches for quote marks (\" or \')
...
`py-strtype', indicating what sort of string begins immediately after
...
quote mark that opened the string."
  (when py-syntactic-triple-quoted-strings
    (let (result strtype)
      ;; Find a known starting place & state.
      (cond ((= (point) (car py-strtype-cache))
             (setq strtype (cdr py-strtype-cache)))
            ((re-search-backward "[\"\']" nil t)
             (setq strtype (get-text-property (point) 'py-strtype))
             (forward-char 1))
            (t nil))
      ...
      (while (not result)
        ...
        (if (re-search-forward "[\"\'\\#]" limit t)
            (let ((start (match-beginning 0))
              ...
              (put-text-property start (point) 'py-strtype strtype))
          ;; No tokens left
          (setq result 'reached-limit)))
      (setq py-strtype-cache (cons (point) strtype))
      (eq result 'found-one))))

(defvar py-strtype-cache '(-1 . nil)
...
;; Don't let the 'py-strtype' property spread to other characters.
(when (boundp 'text-property-default-nonsticky)
  (add-to-list 'text-property-default-nonsticky '(py-strtype . t)))

(defun py-fontify-region (beg end &optional loudly)
  "Function to use for fontifying regions in `python-mode'.  It is
identical to `font-lock-default-fontify-region', except that it fixes
a bug that applies to some versions of XEmacs by temporarily setting
`lookup-syntax-properties' to true.  This should have no adverse effects
for versions of Emacs that are not affected by the bug."
...
py-beep-if-tab-change\t\tring the bell if `tab-width' is changed"
  ...
  (setq major-mode              'python-mode
        ...
        font-lock-syntactic-keywords py-font-lock-syntactic-keywords
        font-lock-fontify-region-function 'py-fontify-region
        paragraph-separate      "^[ \t]*$"
        paragraph-start         "^[ \t]*$"
